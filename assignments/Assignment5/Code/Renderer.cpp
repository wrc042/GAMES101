...
Vector3f castRay(const Vector3f &orig, const Vector3f &dir, const Scene &scene,
    ...
    if (auto payload = trace(orig, dir, scene.get_objects()); payload) {
        ...
        case REFLECTION: {
            ...
            Vector3f reflectionRayOrig =
                (dotProduct(reflectionDirection, N) < 0)
                    ? hitPoint + N * scene.epsilon
                    : hitPoint - N * scene.epsilon;
            hitColor = castRay(reflectionRayOrig, reflectionDirection, scene,
                               depth + 1) *
                       kr;
            break;
        }
        default: {
            ...
            // model is composed of a diffuse and a specular reflection
            // component.
            // [/comment]
            ...
            for (auto &light : scene.get_lights()) {
                ...
                bool inShadow =
                    shadow_res &&
                    (shadow_res->tNear * shadow_res->tNear < lightDistance2);
...
void Renderer::Render(const Scene &scene) {
    ...
    for (int j = 0; j < scene.height; ++j) {
        for (int i = 0; i < scene.width; ++i) {
            ...
            float dir_l = sqrt((dir.x * dir.x) + (dir.y * dir.y + 1));
            dir = dir / dir_l;
