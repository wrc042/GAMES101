...
static bool insideTriangle(int x, int y, const Vector3f *_v) {
    ...
    float side_1 = (x - _v[0].x()) * (_v[1].y() - _v[0].y()) -
                   (y - _v[0].y()) * (_v[1].x() - _v[0].x());
    float side_2 = (x - _v[1].x()) * (_v[2].y() - _v[1].y()) -
                   (y - _v[1].y()) * (_v[2].x() - _v[1].x());
    float side_3 = (x - _v[2].x()) * (_v[0].y() - _v[2].y()) -
                   (y - _v[2].y()) * (_v[0].x() - _v[2].x());
    return (side_1 * side_2 >= 0) && (side_2 * side_3 >= 0);
}
...
void rst::rasterizer::rasterize_triangle(const Triangle &t) {
    ...
    // iterate through the pixel and find if the current pixel is inside
    // the triangle

    // If so, use the following code to get the interpolated z value.
    // auto[alpha, beta, gamma] = computeBarycentric2D(x, y, t.v);
    // float w_reciprocal = 1.0/(alpha / v[0].w() + beta / v[1].w() + gamma
    // / v[2].w()); float z_interpolated = alpha * v[0].z() / v[0].w() +
        ...
        for (int y = y_min; y <= y_max; y++) {
            if (insideTriangle(x, y, t.v)) {
                ...
                if (z_interpolated < depth_buf[index]) {
                    ...
                    auto color_interpolated =
                        (alpha * t.color[0] + beta * t.color[1] +
                         gamma * t.color[2]) *
                        255;
                    set_pixel(Vector3f(x, y, 0), color_interpolated);
